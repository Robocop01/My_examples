...
if (isset($_POST['new']))
   mysql_query("INSERT INTO sn_printer SET id_s_m='".$_POST['id_s_m']."',
   id_printer='".$_POST['id_printer']."', n_sn='".$_POST['n_sn']."', d_y='".$_POST['d_y']."', note='".$_POST['note']."'");

 if (isset($_POST['delete']))
   mysql_query("DELETE FROM sn_printer WHERE id_sn='".$_SESSION['id_sn']."'");
...
<?
  $q1=mysql_query("SELECT n_management, n_printer, id_sn, n_sn, d_y, n_station, sn_printer.note
  FROM user_resource, manag_state, sn_printer, f_printer, management, station
  WHERE id_user='".$_SESSION['id_user']."'
  AND user_resource.id_s_m=manag_state.id_s_m
  AND manag_state.id_s_m=sn_printer.id_s_m
  ...
  AND sn_printer.id_printer=f_printer.id_printer
   Order By n_station, n_management, n_printer, n_sn");
...
<tr><th colspan="7"><font size="4">Принтеры по S/N</font></th></tr>
<tr>
...
echo '<td align="left"><font size="4">'.mysql_result($q1, $i, 'n_printer').'</font></td>';
echo '<td align="left"><a href="edit_sn_printer.php?id_sn='.mysql_result($q1, $i, 'id_sn').'">
